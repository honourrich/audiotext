-- Fix RLS Security Issues - Ultra Safe Version
-- This migration enables RLS on all tables and creates policies only for tables that exist with correct columns
...
-- Create basic policies for tables with user_id column
...
-- Episode collaborators - only if episodes table exists with user_id
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'episode_collaborators' AND table_schema = 'public') THEN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'episodes' AND table_schema = 'public') THEN
            IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'episodes' AND column_name = 'user_id' AND table_schema = 'public') THEN
...
-- Version history - only if episodes table exists with user_id
DO $$
BEGIN
...
-- Episode comments - only if episodes table exists with user_id
DO $$
BEGIN
...
-- Episode reactions - only if episodes table exists with user_id
DO $$
BEGIN
...
-- Workflow states - only if episodes table exists with user_id
DO $$
BEGIN
